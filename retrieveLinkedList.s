/*
 * File: retrieveLinkedList.s
 * Author: Yingjian Pei
 * Userid: cs30fex
 * Description: file that can call table->hashFunction() to hash string
 * Then uses the result as the index into llArray
 * Date: Nov 7, 2018
...
@ store numbers
        .global retrieveLinkedList      @ Specify it as a global
        .equ    FOUR, 4                 @ four: 4
        .equ    leftS,2                 @ lsl: 2
@ Text region   
        ...
        .align 2                        @ Align
/*
 * Function Header: retrieveLinkedList()
 * Function Prototype: linkedList_t *retrieveLinkedList
 *                     ( table_t *table, char *string )
 * Description: Call table->hashFunction() to hash string. Note that the 
 * hashkey returned by each of the hash functions will not necessarily
 * be an index within the size of the hash table.
 * Parameters: retrieveLinkedList*: the pointer 
 * Side Effects: none
 * Return values: void
 * Registers used:
...
retrieveLinkedList:
       ...
       //get table -> llArray[index]
        ...
        ldr     r0,[r0]                 @ get the content of local offset
